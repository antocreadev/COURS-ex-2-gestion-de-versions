...
#
# Usage :
#   ./scripts/creer-mon-depot.sh <nom-du-depot> [login-du-binome]
#
# Exemple :
...
#   3. ajoute votre binôme en collaborateur (s'il est indiqué)
...
if [ -z "$NOM" ]; then
  echo "Usage : $0 <nom-du-depot> [login-du-binome]" >&2
  exit 1
fi
...
# --- 3. Binôme -------------------------------------------------------------
if [ -n "$BINOME" ]; then
  gh api -X PUT "repos/$DEPOT/collaborators/$BINOME" -f permission=push >/dev/null
  echo "→ $BINOME invité en collaborateur (droit d'écriture)"
fi
...
# Sans binôme, on n'exige AUCUNE approbation : GitHub interdit d'approuver ses
# propres PR, vous seriez bloqué. La CI, elle, reste obligatoire dans tous les cas.
APPROBATIONS=0
[ -n "$BINOME" ] && APPROBATIONS=1
